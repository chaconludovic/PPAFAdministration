 ...
 */
public class ItemLazyDataModel extends GenericLazyDataModel<Item> {
    private static final long serialVersionUID = 1L;

    @Inject
    transient private ItemRepository itemRepository;
    @Inject
    transient private ItemSearchForm itemSearchForm;

    /**
     * Prepare the search parameters and call the itemRepository finder.
     * Automatically called by PrimeFaces component.
     */
    @Override
    public List<Item> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        SearchParameters sp = itemSearchForm.getSearchParameters();

        Item item = itemSearchForm.getItem();
        setRowCount(itemRepository.findCount(item, sp)); // total count so the paginator may display the total number of pages
        populateSearchParameters(sp, first, pageSize, sortField, sortOrder, filters); // load one page of data

        return itemRepository.find(item, sp);
    }
}